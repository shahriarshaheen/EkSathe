...
router.post("/routes/:id/deviation-alerts/:alertId/ack",authenticate, acknowledgeAlert);

// ─── F-18: Check-In ───────────────────────────────────────────────────────────
router.post("/:id/checkin",              authenticate, submitCheckin);
router.get("/:id/checkin-status",        authenticate, getCheckinStatus);
router.patch("/:id/auto-cancel-no-show", authenticate, autoCancelNoShows);

// ─── Admin only ───────────────────────────────────────────────────────────────
router.get("/admin/routes",    authenticate, authorize("admin"), adminGetAllRoutes);
router.patch("/admin/routes/:id/cancel", authenticate, authorize("admin"), adminCancelRoute);
