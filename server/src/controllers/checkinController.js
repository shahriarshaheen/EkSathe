...
import { awardCheckinCoins } from "../services/rewardService.js";
import {
  getCheckinWindow,
  isCheckinWindowOpen,
  isDeparturePasssed,
...
export const submitCheckin = async (req, res) => {
  try {
    ...
    const existingCheckin = ride.checkins?.find(
    ...
    if (existingCheckin) {
      return res.status(200).json({
        ...
        reward: {
          coinsEarned: existingCheckin.coinsEarned || 0,
          coinBalance: null,
          reason: existingCheckin.rewardReason || "Already checked in",
        },
      });
    }
    ...
    const reward = await awardCheckinCoins({
      userId,
      ride,
      checkedInAt,
      checkinLocation: hasLocation
        ? {
            lat: Number(lat),
            lng: Number(lng),
          }
    ...
    ride.checkins.push({
      ...
      distanceFromPickupMeters: reward.distance ?? null,
      coinsEarned: reward.coinsEarned || 0,
      coinTransactionId: reward.transactionId || null,
    ...
    const message =
      reward.coinsEarned > 0
        ? `Check-in confirmed! You earned ${reward.coinsEarned} coins.`
        : "Check-in confirmed! See you at the pickup point.";
    ...
    return res.status(200).json({
      ...
      reward: {
        coinsEarned: reward.coinsEarned || 0,
        coinBalance: reward.coinBalance ?? null,
  ...
  try {
    ...
    const passengerStatuses = ride.passengers.map((p) => {
      ...
      return {
        ...
        coinsEarned:  checkin?.coinsEarned ?? 0,
        rewardReason: checkin?.rewardReason ?? null,
      };
    });
