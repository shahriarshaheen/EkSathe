...
import {
  reserveCoinsForPayment,
  completeCoinReservation,
...
export const initiateCarpoolPayment = async (req, res) => {
  ...
  try {
    ...
    // Coupon / coin discount re-validation
    let finalAmount = originalAmount;
    let discountAmount = 0;
    let couponCode = req.body.couponCode || null;
    const coinTierId = req.body.coinTierId || null;

    let discountSource = null;
    let coinDiscountAmount = 0;
    let coinsRedeemed = 0;
    let coinTransactionId = null;
    let coinReservationStatus = "none";

    if (couponCode && coinTierId) {
      return res.status(400).json({
        success: false,
        message: "Use either a coupon or coins, not both",
      });
    }
    ...
    if (coinTierId) {
      try {
        const reservation = await reserveCoinsForPayment({
          userId: req.user.id,
        ...
        coinDiscountAmount = reservation.tier.discountAmount;
        coinsRedeemed = reservation.tier.coinsRequired;
        discountAmount = coinDiscountAmount;
        finalAmount = Math.max(0, originalAmount - coinDiscountAmount);

        createdPayment.amount = finalAmount;
        createdPayment.discountAmount = discountAmount;
        createdPayment.couponCode = null;
        createdPayment.discountSource = "coins";
        createdPayment.coinDiscountAmount = coinDiscountAmount;
        createdPayment.coinsRedeemed = coinsRedeemed;
        createdPayment.coinTransactionId = reservation.transaction._id;
        createdPayment.coinReservationStatus = "reserved";

        await createdPayment.save();
      } catch (coinErr) {
        createdPayment.status = "failed";
        createdPayment.gatewayResponse = { error: coinErr.message };
        await createdPayment.save();

        return res.status(400).json({
          success: false,
          message: coinErr.message || "Unable to redeem coins",
        });
      }
    }
    if (finalAmount <= 0) {
      createdPayment.status = "paid";
      createdPayment.amount = 0;
      createdPayment.gatewayResponse = {
        source: "coins",
        message: "Fully paid using reward coins",
      };

      if (createdPayment.coinReservationStatus === "reserved") {
        await completeCoinReservation(createdPayment);
        createdPayment.coinReservationStatus = "completed";
      }
      ...
      return res.status(200).json({
        success: true,
        url: `${getSSL().CLIENT_URL}/payment/success?tran_id=${tranId}&type=carpool`,
      });
    }
    const sslcz = new SSLCommerzPayment(STORE_ID, STORE_PASSWORD, IS_LIVE);
    const apiResponse = await sslcz.init(data);
    ...
    if (createdPayment?.coinReservationStatus === "reserved") {
      await releaseCoinReservation(createdPayment, "SSLCommerz initiation failed");
      createdPayment.coinReservationStatus = "released";
      await createdPayment.save();
    }
    ...
    if (createdPayment) {
      try {
        ...
        if (createdPayment.coinReservationStatus === "reserved") {
          await releaseCoinReservation(createdPayment, "Payment initiation error");
          createdPayment.coinReservationStatus = "released";
        }
...
export const carpoolPaymentSuccess = async (req, res) => {
  ...
  try {
    ...
    if (
      !wasAlreadyPaid &&
      payment.coinReservationStatus === "reserved" &&
      payment.coinTransactionId
    ) {
      await completeCoinReservation(payment);
      payment.coinReservationStatus = "completed";
    }
  ...
  try {
    ...
    if (payment) {
      if (
        ...
        await releaseCoinReservation(payment, "Carpool payment failed");
        payment.coinReservationStatus = "released";
        await payment.save();
      }

      await releaseCarpoolSeat(payment);
    }
  ...
  try {
    ...
    if (payment) {
      if (
        ...
        await releaseCoinReservation(payment, "Carpool payment cancelled");
        payment.coinReservationStatus = "released";
        await payment.save();
      }
  ...
  try {
    ...
    if (isGatewayValid(status)) {
      ...
      if (gatewayValid) {
        ...
        if (payment.contextType === "parking") {
          await confirmParkingBooking(payment);
        }
