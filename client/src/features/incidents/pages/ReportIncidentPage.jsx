    ...
    <div className="min-h-screen bg-stone-50 flex items-start justify-center px-4 py-12">
      <div className="w-full max-w-2xl">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-semibold text-stone-800">Report an Incident</h1>
            <p className="text-sm text-stone-500">Your identity will remain anonymous</p>
          </div>
        </div>

        <div className="bg-white border border-stone-200 rounded-xl p-8 shadow-sm">
          <div className={`flex items-center gap-2 text-sm mb-6 px-3 py-2 rounded-lg
            ${gpsStatus === "granted" ? "bg-teal-50 text-teal-700" :
              gpsStatus === "denied" ? "bg-amber-50 text-amber-700" :
              "bg-stone-100 text-stone-500"}`}>
            <MapPin className="w-4 h-4 flex-shrink-0" />
            {gpsStatus === "granted" && "GPS location captured automatically"}
            {gpsStatus === "denied" && "GPS unavailable — you can type your location below"}
