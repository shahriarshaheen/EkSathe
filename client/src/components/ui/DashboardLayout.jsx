...
import { MapPin, LogOut, Menu, X, UserCircle, Bell, Coins } from "lucide-react";
...
const DashboardLayout = ({ children, navItems = [] }) => {
  ...
  const effectiveNavItems =
    user?.role === "student" &&
    !navItems.some((item) => item.path === "/dashboard/rewards")
      ? [
          ...navItems,
          {
            label: "My Coins",
  ...
  const renderNavItem = (item) => {
    const isBell = item.icon === Bell || item.label === "Notifications";
