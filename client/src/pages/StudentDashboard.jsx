...
import {
  MapPin, Car, Shield, Bell, Home,
  Navigation, Star, Clock,
} from "lucide-react";
  ...
  useEffect(() => {
    api.get("/carpool/my")
      .then((r) => {
        ...
        const posted = r.data.data.posted?.filter(
        (r) => r.status !== "cancelled" && r.status !== "completed"
        ).length || 0;
        const joined = r.data.data.joined?.filter(
        (r) => r.status !== "cancelled" && r.status !== "completed"
        ).length || 0;
    ...
    <DashboardLayout navItems={navItems}>
      <div className="p-6 lg:p-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          />
          <StatCard label="Trust Score"   value={user?.trustScore ?? 0}  sub="Build your score" accent="text-teal-600" />
          <StatCard label="Campus Safety" value="Active"                  sub="SOS available"    accent="text-green-600" />
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-teal-800">
                  You have {ridesShared} active carpool ride{ridesShared !== 1 ? "s" : ""}
                </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-xl border border-stone-200 p-5">
            <h3 className="text-sm font-semibold text-stone-700 mb-4">Recent Activity</h3>
            <ActivityRow icon={Star}   text="Account created successfully" time="Just now" color="bg-teal-50 text-teal-500" />
            <ActivityRow icon={Shield} text="Email verified"               time="Just now" color="bg-green-50 text-green-500" />
            <div className="pt-4 text-center">
              <p className="text-xs text-stone-400">Your activity will appear here as you use EkSathe.</p>
            </div>
          </div>

          <div className="bg-white rounded-xl border border-stone-200 p-5">
            <h3 className="text-sm font-semibold text-stone-700 mb-4">Your Profile</h3>
            <div className="flex flex-col items-center text-center gap-3">
              ...
              {user?.studentId && (
                <div className="w-full bg-stone-50 rounded-lg px-3 py-2">
                  ...
                  <p className="text-sm font-mono font-medium text-stone-700">{user?.studentId}</p>
                </div>
              )}
              <div className="w-full bg-teal-50 rounded-lg px-3 py-2">
                <p className="text-xs text-teal-500">Trust Score</p>
                <p className="text-lg font-bold text-teal-700">{user?.trustScore ?? 0}</p>
              </div>
