...
function RouteCard({ route, onJoin, onLeave, onCancel, joining, leaving, currentUserId }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden ${isCancelled ? "border-red-100 opacity-75" : "border-stone-200"}`}>
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="flex flex-col items-end gap-2 flex-shrink-0">
            <div className={`text-lg font-black ${isCancelled ? "text-stone-300 line-through" : isFull ? "text-orange-500" : "text-teal-600"}`}>
              {TAKA}{route.pricePerSeat}
            </div>
            <div className="flex flex-col items-end gap-1">
              {route.genderSafe && (
                <span className="bg-pink-50 border border-pink-200 text-pink-600 rounded-full px-2 py-0.5 text-xs font-semibold">Female only</span>
              )}
        ...
        {/* Info strip */}
        <div className="flex items-center gap-3 py-3 border-t border-b border-stone-50 mb-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-stone-700">{route.availableSeats} of {route.totalSeats}</span>
        ...
        {route.driver && (
          <div className="flex items-center gap-3 mb-4">
            {route.driver.photoUrl ? (
              <img src={route.driver.photoUrl} alt={route.driver.name}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-stone-800">{route.driver.name}</p>
        ...
        {route.notes && (
          <div className="bg-stone-50 rounded-xl px-3 py-2 mb-4 flex items-start gap-2">
            ...
            <p className="text-xs text-stone-500 italic leading-relaxed">{route.notes}</p>
          </div>
        )}
        ...
        {isDriver ? (
          <div className="flex flex-col gap-2">
            ...
            {!isCancelled && (
              <button onClick={() => onCancel(route._id)}
          ...
          <div className="w-full py-3 rounded-xl bg-stone-50 border border-stone-200 text-sm font-semibold text-stone-400 text-center">
            Ride cancelled
          </div>
        ) : isPassenger ? (
          // Fix 19 — leave ride button
          <div className="flex flex-col gap-2">
  ...
  return (
    <DashboardLayout navItems={navItems}>
      <div className="min-h-screen bg-stone-50">
        ...
        {/* Hero — Fix 14: stone palette */}
        <div className="bg-gradient-to-br from-teal-700 via-teal-600 to-teal-500 pt-8 pb-20 px-4 relative overflow-hidden">
          ...
          <div className="max-w-2xl mx-auto relative">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                <button onClick={() => navigate("/dashboard/carpool/my-rides")}
                  className="text-teal-200 hover:text-white text-xs font-medium transition-colors underline">
                  My Rides
                </button>
              </div>
            </div>
            ...
            {/* Mode toggle */}
            <div className="flex bg-teal-800/40 rounded-xl p-1 mb-4">
            ...
            {/* Preset search — Fix 17: fade gradient on pills */}
            {searchMode === "preset" && (
              <div className="flex flex-col gap-3">
                <div className="relative">
                  ...
                  {/* Fix 17 — fade gradient indicator */}
                  <div className="absolute right-0 top-0 bottom-1 w-8 bg-gradient-to-l from-teal-600 to-transparent pointer-events-none"/>
                </div>
                <div className="relative">
                  <select value={selectedPreset} onChange={(e) => setSelectedPreset(e.target.value)}
                    className="w-full bg-white/95 rounded-xl border-0 px-4 py-3.5 text-sm text-stone-800 font-medium focus:outline-none focus:ring-2 focus:ring-white/50 appearance-none shadow-lg">
              ...
              <div className="flex flex-col gap-3">
                <div className="relative">
                  ...
                  <input value={customOrigin} onChange={(e) => setCustomOrigin(e.target.value)}
                    ...
                    className="w-full bg-white/95 rounded-xl border-0 pl-8 pr-4 py-3.5 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-white/50 shadow-lg"/>
                </div>
                <div className="relative">
        ...
        <div className="max-w-2xl mx-auto px-4 -mt-12 pb-8 relative z-10">
          ...
          {/* Filter bar — Fix 14: stone palette */}
          <div className="bg-white rounded-2xl shadow-lg border border-stone-200 px-4 py-3 mb-4 flex items-center gap-3">
            <label className="flex items-center gap-2 text-sm text-stone-600 cursor-pointer select-none">
              <div onClick={() => setGenderSafe(!genderSafe)}
                className={`w-9 h-5 rounded-full transition-all relative flex-shrink-0 cursor-pointer ${genderSafe ? "bg-pink-500" : "bg-stone-200"}`}>
          ...
          {/* Alerts */}
          {joinSuccess && (
            <div className="rounded-2xl bg-teal-50 border border-teal-200 px-5 py-4 text-sm text-teal-700 mb-4 flex items-center gap-3">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-sm text-stone-400 font-medium">Finding rides...</p>
            </div>
          ) : routes.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 flex flex-col items-center justify-center py-16 text-center px-6">
