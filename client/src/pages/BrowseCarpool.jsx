...
import CoinDiscountPanel from "../components/CoinDiscountPanel";
...
function RideCheckoutModal({ route, joining, onClose, onConfirmOnline, onConfirmCash }) {
  ...
  const [selectedCoinTier, setSelectedCoinTier] = useState(null);

  const coinDiscount = selectedCoinTier?.discountAmount || 0;
  const couponDiscount = couponState?.discountAmount || 0;

  const finalAmount = Math.max(
    0,
    couponState?.finalAmount ?? route.pricePerSeat - coinDiscount,
  );
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      ...
      >
        ...
        <div className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <CoinDiscountPanel
            amount={route.pricePerSeat}
            selectedTierId={selectedCoinTier?.id}
            onSelectTier={(tier) => {
              setSelectedCoinTier(tier);
              if (tier) setCouponState(null);
            }}
          ...
          <div className="space-y-2 pt-1">
            ...
            <p className="text-xs text-stone-400 text-center">
              Cash payment: pay the driver directly when you board. Discounts apply to online payment only.
            </p>
          </div>
        </div>
      </motion.div>
...
export default function BrowseCarpool() {
  ...
  const handleConfirmOnline = async (route, couponCode = null, coinTierId = null) => {
